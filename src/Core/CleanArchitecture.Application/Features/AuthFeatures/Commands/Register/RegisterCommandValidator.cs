...
public sealed class RegisterCommandValidator : AbstractValidator<RegisterCommand>
{
    ...
    {
        RuleFor(p => p.Email).NotEmpty().WithMessage("Email field cannot be empty!");
        RuleFor(p => p.Email).NotNull().WithMessage("Email field cannot be empty!");
        RuleFor(p => p.Email).EmailAddress().WithMessage("Enter a valid email address!");
        ...
        RuleFor(p => p.UserName).MinimumLength(3).WithMessage("Username must be at least three characters!");
